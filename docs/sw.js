(() => {
  ...
  // sw/system/readTokens.js
  var readTokens = async () => {
    let tokens;
    try {
      tokens = await import_localforage.default.getItem(LOCAL_TOKENS_KEY);
    } catch (e) {
  ...
  var readTokens_default = readTokens;
  ...
  async function getList() {
    ...
    const tokens = await readTokens_default();
    const result = await requestJson_default("https://content.googleapis.com/calendar/v3/calendars/primary/events", {
      searchParams: {
        calendarId: "primary",
  ...
  var refreshTokens = async () => {
    ...
    await import_localforage2.default.setItem(LOCAL_TOKENS_KEY, tokens);
    return tokens;
  };
  var refreshTokens_default = refreshTokens;

  // sw/system/readUser.js
  var import_localforage3 = __toESM(require_localforage(), 1);
  var readUser = async () => {
    let user;
    try {
      user = await import_localforage3.default.getItem(LOCAL_USER_KEY);
    } catch (e) {
      throw new Error("No cached user!");
    }
    assume_default(checkPojo_default(user), "Unexpected user type!");
    assume_default(user.email, "Unexpected user shape!");
    return user;
  };
  var readUser_default = readUser;
  ...
  var getUser = async () => {
    ...
    let tokens = await readTokens_default();
    let user = await getUserProfileData(tokens.access_token);
    if (user.error) {
      tokens = await refreshTokens_default();
      user = await getUserProfileData(tokens.access_token);
    }
    assume_default(!user.error, user.error + ": " + user.error_description);
    assume_default(user.email, "Missing email from user info!");
    await import_localforage4.default.setItem(LOCAL_USER_KEY, user);
    return user;
  };
  var getUserProfileData = async (accessToken) => {
    const user = await requestJson_default("https://www.googleapis.com/oauth2/v3/userinfo", {
      headers: {
  ...
  var setTokens = async (request) => {
    ...
    assume_default(tokens.access_token && tokens.refresh_token, "Unexpected tokens shape!");
    await import_localforage5.default.setItem(LOCAL_TOKENS_KEY, tokens);
    return true;
  };
  var setTokens_default = setTokens;

  // sw/sw.js
  var CACHED_PATHS = ["/","/index.html","/js/main-DUmx3LNQ.js","/js/main-DUmx3LNQ.js.map","/manifest.json","/meta/192.png","/meta/512.png","/meta/apple-splash-1080-2340.png","/meta/destiny-1024.png","/meta/destiny.png","/meta/destiny.svg"];
